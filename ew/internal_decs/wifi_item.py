...
from sdk.widgets.selectable_item import SelectableItem
from ew.util import ew_logging


logger = ew_logging.getLogger('ew.internal_decs.wifi_item')

class WifiItem(SelectableItem):

    def __init__(self, width, height, widget_id=None):
        SelectableItem.__init__(self, width, height)
        self.start_x = 4
        self.start_y = 4
        self.spacer = 4
        self.security_type = "Open"
        self.known = False
        self.connected = False
        
    def create_gui(self, network_page):
        self.name  = Label('', self.w/2, self.h, 16)
    ...
    def set_from_data(self, net):
        ...
        for item in supported_network:
            if item in net[2]:
                self.security_type = item
                break
        if self.security_type != "Open":
